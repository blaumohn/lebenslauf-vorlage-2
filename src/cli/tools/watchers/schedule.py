...
def schedule_yaml(observer, process_env, root_path, demo, yaml_path, yaml_dir, build_fn):
    if yaml_path:
        watch_dir = os.path.dirname(yaml_path)
        patterns = [yaml_path]
        observer.schedule_watch(
            watch_dir, patterns, lambda: build_fn(process_env, root_path, demo)
        )
    ...
    if yaml_dir and os.path.isdir(yaml_dir):
        ...
        observer.schedule_watch(
            yaml_dir, patterns, lambda: build_fn(process_env, root_path, demo)
        )
...
def schedule_twig(observer, process_env, root_path, demo, build_fn):
    if not twig.enabled():
    ...
    observer.schedule_watch(
        watch_dir,
        patterns,
        lambda: build_fn(process_env, root_path, demo),
        recursive=True,
    )
